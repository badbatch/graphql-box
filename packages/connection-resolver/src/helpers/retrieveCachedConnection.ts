...
export const retrieveCachedConnection = async (
  ...
  // is called from `resolveConnection` and is only called if there is a cursor. It is
  // called either after a direct check or after calling `validateCursor` or `requestAndCachePages`,
  ...
  const entry = retrieveEntry(args, metadata, { cursorCache, resultsPerPage });
